    ...
    startNodes(() => {
      ...
      groupsTemplate(ncdcConfig).put(ncdcConfig, ncdcGroup, (e, v) => {
        ...
        groupsTemplate(nodeConfig).put(nodeConfig, nodeGroup, (e, v) => {
          ...
          groupsTemplate(
              crawlerConfig).put(crawlerConfig, crawlerGroup, (e, v) => {
            const extractConfig = {gid: 'extract'};
            groupsTemplate(
                extractConfig).put(extractConfig, extractGroup, (e, v) => {
              const regexConfig = {gid: 'regex'};
              groupsTemplate(
                  regexConfig).put(regexConfig, regexGroup, (e, v) => {
                done();
              });
            });
          });
  ...
  let m1 = (key, url) => {
    return new Promise((resolve) => {
      global.https.get(url, (res) => {
        ...
        res.on('end', () => {
          global.distribution.extract.store.put(page, url, ()=>{
            let out = {};
            out[url] = 1;
            resolve(out);
          });
...
test('url extraction workflow', (done) => {
  ...
  let expected = [
    {'https://www.york.ac.uk/teaching/cws/wws/webpage1.html': 4},
    {'https://www.york.ac.uk/teaching/cws/wws/webpage2.html': 3},
    {'https://www.york.ac.uk/teaching/cws/wws/webpage3.html': 2},
    {'https://www.york.ac.uk/teaching/cws/wws/webpage4.html': 2},
    {'https://www.york.ac.uk/teaching/cws/wws/col3.html': 4},
    {'https://www.york.ac.uk/inst/cws/': 1},
    {'http://trace.ntu.ac.uk/mentors/madigan/colours.htm': 1},
    {'http://www.umbc.edu/cwit/': 1},
    {'mailto:eakn1@york.ac.uk': 1},
  ];
  ...
  /* Now we do the same thing but on the cluster */
  const configuration = {keys: keys, map: m1, reduce: r1, memory: true};
  distribution.extract.mr.exec(configuration, (e, v) => {
    try {
      expect(v).toEqual(expect.arrayContaining(expected));
      done();
    } catch (e) {
